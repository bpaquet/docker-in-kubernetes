...
// `?until=<past>` closes the stream immediately. Covers both timestamp formats.
func TestEventsUntilInPastReturnsImmediately(t *testing.T) {
	tests := map[string]string{
		"unix seconds": strconv.FormatInt(time.Now().Add(-time.Hour).Unix(), 10),
		"RFC3339":      time.Now().Add(-time.Hour).UTC().Format(time.RFC3339),
	}
	for name, until := range tests {
		t.Run(name, func(t *testing.T) {
			ts := newTestServer(t)

			resp, err := http.Get(ts.URL + "/v1.43/events?until=" + until) //nolint:bodyclose // closed via defer below
